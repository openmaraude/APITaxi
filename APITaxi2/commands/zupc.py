...
from APITaxi_models2 import db, Departement, ADS, ZUPC
...
def recreate_zupc_tmp_table():
    ...
    current_app.logger.info('Create temporary table %s' % table.name)
...
def fill_zupc_tmp_table_from_contours(shape_filename):
    ...
    for geom, properties in get_records_from_shapefile(shape_filename):
        ...
        if not departement:
            current_app.logger.warning('Unable to find departement of INSEE code %s', properties['insee'])
            continue
...
def fill_zupc(zupc_dir):
    ...
    if not os.path.exists(insee_list_filename):
        current_app.logger.debug('No insee.list file in %s, nothing to do', zupc_dir)
        return
...
def merge_zupc_tmp_table():
    ...
    current_app.logger.info('Update ZUPC.parent_id of %s rows', query.count())
    for row in query:
    ...
    current_app.logger.info('Update ADS.zupc_id of %s rows', query.count())
    for ads in query:
...
@click.option(
    '--contours-tmpdir', default=CONTOURS_DEFAULT_TMPDIR,
    help='Where --contours-url is downloaded and extracted, default=%s' % CONTOURS_DEFAULT_TMPDIR
)
@click.option(
    '--zupc-repo', default=ZUPC_DEFAULT_DIRECTORY,
