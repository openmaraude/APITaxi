...
def upgrade():
    ...
    op.execute(hail.update().where(hail.c.rating_ride_reason==u'no_credit_card')
                .values(rating_ride_reason='payment'))
    op.execute(hail.update().where(hail.c.rating_ride_reason==u'bad_itinerary')
                 .values(rating_ride_reason='route'))
    op.execute(hail.update().where(hail.c.rating_ride_reason==u'dirty_taxi')
                 .values(rating_ride_reason='cleanliness'))
    op.execute(hail.update().where(hail.c.rating_ride_reason==u'late')
                 .values(rating_ride_reason='route'))
    ...
    op.execute(hail.update().where(hail.c.reporting_customer_reason==u'late')
                 .values(reporting_customer_reason='route'))
    op.execute(hail.update().where(hail.c.reporting_customer_reason==u'aggressive')
                  .values(reporting_customer_reason='courtesy'))
    op.execute(hail.update().where(hail.c.reporting_customer_reason==u'no_show')
    ...
    op.execute(hail.update().where(hail.c.incident_taxi_reason==u'traffic_jam')
                  .values(incident_taxi_reason='traffic'))
    op.execute(hail.update().where(hail.c.incident_taxi_reason==u'garbage_truck')
                   .values(incident_taxi_reason='traffic'))
    ...
    op.execute(hail.update().where(hail.c.incident_customer_reason==u'mud_river')
                  .values(incident_customer_reason=''))
    op.execute(hail.update().where(hail.c.incident_customer_reason==u'parade')
                  .values(incident_customer_reason=''))
    op.execute(hail.update().where(hail.c.incident_customer_reason==u'earthquake')
                  .values(incident_customer_reason=''))


def downgrade():
    op.execute(hail.update().where(hail.c.rating_ride_reason==u'payment')
                 .values(rating_ride_reason='no_credit_card'))
    op.execute(hail.update().where(hail.c.rating_ride_reason==u'route')
                 .values(rating_ride_reason='bad_itinerary'))
    op.execute(hail.update().where(hail.c.rating_ride_reason==u'cleanliness')
                 .values(rating_ride_reason='dirty_taxi'))
    op.execute(hail.update().where(hail.c.rating_ride_reason==u'courtesy')
                 .values(rating_ride_reason='dirty_taxi'))
    op.execute(hail.update().where(hail.c.rating_ride_reason==u'ko')
                 .values(rating_ride_reason='dirty_taxi'))
    ...
    op.execute(hail.update().where(hail.c.reporting_customer_reason==u'route')
                  .values(reporting_customer_reason='late'))
    op.execute(hail.update().where(hail.c.reporting_customer_reason==u'courtesy')
                  .values(reporting_customer_reason='aggressive'))
    op.execute(hail.update().where(hail.c.reporting_customer_reason==u'cleanliness')
                  .values(reporting_customer_reason='late'))
    op.execute(hail.update().where(hail.c.reporting_customer_reason==u'payment')
                  .values(reporting_customer_reason='no_show'))
    op.execute(hail.update().where(hail.c.reporting_customer_reason==u'ko')
                  .values(reporting_customer_reason='no_show'))
    ...
    op.execute(hail.update().where(hail.c.incident_taxi_reason==u'traffic')
                   .values(incident_taxi_reason='traffic_jam'))
    op.execute(hail.update().where(hail.c.incident_taxi_reason==u'address')
                    .values(incident_taxi_reason='traffic_jam'))
    op.execute(hail.update().where(hail.c.incident_taxi_reason==u'breakdown')
                    .values(incident_taxi_reason='traffic_jam'))
    op.execute(hail.update().where(hail.c.incident_taxi_reason==u'no_show')
                    .values(incident_taxi_reason='traffic_jam'))
    ...
    op.execute(hail.update().where(hail.c.incident_customer_reason==u'')
